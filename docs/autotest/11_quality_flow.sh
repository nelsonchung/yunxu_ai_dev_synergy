...
payload="$(
  cat <<JSON
{"title":"品質測試${timestamp}","companyName":"測試公司","projectType":"新系統建置","background":"需求背景","goals":"專案目標","scope":"功能範圍","constraints":"","budgetRange":"50 - 150 萬","timeline":"1-2 個月","specDoc":"尚未準備","attachments":[],"contact":{"name":"測試者","email":"tester${timestamp}@example.com","phone":""}}
JSON
)"
...
project_payload="$(printf '{"requirementId":"%s","name":"%s"}' "$requirement_id" "品質測試專案")"
...
log_step "generate testing"
test_payload="$(printf '{"project_id":"%s","scope":"核心流程"}' "$project_id")"
response="$(request_json "POST" "/api/testing/generate" "$test_payload" "$admin_cookie" "")"
parse_response "$response"
assert_status "200" "$RESPONSE_STATUS" "generate testing"

log_step "request code review"
review_payload="$(printf '{"project_id":"%s","repository_url":"https://example.com/repo","commit_sha":"abc123"}' "$project_id")"
response="$(request_json "POST" "/api/review/code" "$review_payload" "$admin_cookie" "")"
parse_response "$response"
assert_status "200" "$RESPONSE_STATUS" "request code review"

log_step "list quality reports"
response="$(request_json "GET" "/api/quality/reports?project_id=$project_id" "" "$admin_cookie" "")"
parse_response "$response"
assert_status "200" "$RESPONSE_STATUS" "list reports"
...
if len(reports) < 2:
    raise SystemExit("FAIL: expected at least 2 reports")
print(reports[0]["id"])
PY
)"

log_step "get report detail"
response="$(request_json "GET" "/api/quality/reports/$report_id" "" "$admin_cookie" "")"
parse_response "$response"
assert_status "200" "$RESPONSE_STATUS" "get report"
