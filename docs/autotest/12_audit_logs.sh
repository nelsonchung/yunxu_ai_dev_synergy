...
payload="$(
  cat <<JSON
{"title":"稽核測試${timestamp}","companyName":"測試公司","projectType":"新系統建置","background":"需求背景","goals":"專案目標","scope":"功能範圍","constraints":"","budgetRange":"50 - 150 萬","timeline":"1-2 個月","specDoc":"尚未準備","attachments":[],"contact":{"name":"測試者","email":"tester${timestamp}@example.com","phone":""}}
JSON
)"
...
project_payload="$(printf '{"requirementId":"%s","name":"%s"}' "$requirement_id" "稽核測試專案")"
...
log_step "create task (audit event)"
task_payload='{"title":"稽核事件測試","status":"todo"}'
...
log_step "list audit logs"
response="$(request_json "GET" "/api/audit/logs" "" "$admin_cookie" "")"
parse_response "$response"
assert_status "200" "$RESPONSE_STATUS" "list audit logs"
RESPONSE_JSON="$RESPONSE_BODY" python3 - <<'PY'
import json,os
data=json.loads(os.environ["RESPONSE_JSON"])
actions={log.get("action") for log in data.get("logs",[])}
if "TASK_CREATED" not in actions:
    raise SystemExit("FAIL: TASK_CREATED not found in audit logs")
PY
