...
log_step "verify project deleted"
response="$(request_json "GET" "/api/projects" "" "" "")"
parse_response "$response"
assert_status "200" "$RESPONSE_STATUS" "list projects"
RESPONSE_JSON="$RESPONSE_BODY" python3 - "$project_id" <<'PY'
import json,os,sys
data=json.loads(os.environ["RESPONSE_JSON"])
pid=sys.argv[1]
projects=data.get("projects",[])
if any(item.get("id")==pid for item in projects):
    raise SystemExit("FAIL: project still present in list")
PY
