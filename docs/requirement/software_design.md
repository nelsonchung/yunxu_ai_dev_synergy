...
### 8.3 Project 狀態
- `intake -> requirements_signed -> architecture_review -> system_architecture_signed -> software_design_review -> software_design_signed -> quotation_review -> quotation_signed -> implementation -> system_verification_review -> system_verification_signed -> delivery_review -> delivery_signed -> closed`
...
- 文件類型：需求、系統開發、軟體設計、測試、交付等皆納入統一版本控管。
- 角色責任：需求與交付由 customer 簽核，開發與測試文件由 developer 產出並送審。

### 9.2 版本規則與命名策略
- 版本規則：主版本遞增，保留更新說明與審核記錄。
- 命名格式：`projects/{project_id}/{doc_type}/v{version}.md`

### 9.3 簽核流程與稽核記錄
- 簽核結果需寫入 AuditLog，包含簽核者、時間與意見。
- 簽核通過後，狀態更新並推進專案流程狀態機。

### 9.4 版本比對與回溯策略
- 提供版本差異預覽以利簽核與回溯。
- 新版本產生後，舊版本標記為 `archived` 但可追溯檢視。

## 10. AI 協作設計
### 10.1 任務分類與觸發條件
- 任務類型：需求摘要、需求拆解、規格草稿、程式碼審查、測試案例生成。
- 觸發來源：需求提交、文件更新、里程碑進入審查狀態。

### 10.2 同步/非同步執行策略
- 同步：短任務（摘要、簡短拆解）直接回傳結果。
- 非同步：大量分析/測試生成以 AIJob 排程處理並通知結果。

### 10.3 審核與人機協作流程
- AI 結果必須人工確認後方可進入交付流程。
- 審核紀錄需保留採用/退回/修正的理由。

### 10.4 結果追蹤與版本關聯
- AI 產出與最終採用版本需保留關聯。
- AIJob 與 DocumentVersion 以 target_id 互相對應。
...
### 12.2 代表性錯誤碼對照
- 常見錯誤碼：`invalid_input`, `unauthorized`, `forbidden`, `not_found`, `conflict`, `ai_job_failed`。
- 需保留對應訊息與處理建議。
